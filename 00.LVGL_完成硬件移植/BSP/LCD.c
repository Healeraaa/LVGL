...
void LCD_Color_Fill(uint16_t xsta,uint16_t ysta,uint16_t xend,uint16_t yend,uint16_t *color_p)
{
	...
	// 计算区域的宽度和高度
    width = xend - xsta + 1;  // 宽度是x坐标的差值 + 1
    height = yend - ysta + 1;  // 高度是y坐标的差值 + 1
    
    // 计算要填充的总像素数
    uint32_t size = width * height;
    
    // 设置LCD显示区域的起始和结束坐标
    // 偏移量OFFSET_Y用于Y坐标的调整，可能是由于屏幕的某种起始显示位置
    ...
    /* 配置DMA待实现 */
    // // 使用DMA模式进行数据传输
    // // 将color_p指针指向的数据（颜色数据）传输到LCD显示区域
    // HAL_SPI_Transmit_DMA(&hspi1, (uint8_t*)color_p, size);

    // // 等待DMA传输完成
    // // __HAL_DMA_GET_COUNTER(&hdma_spi1_tx)返回剩余未传输的数据数量，若为0，表示数据传输完成
