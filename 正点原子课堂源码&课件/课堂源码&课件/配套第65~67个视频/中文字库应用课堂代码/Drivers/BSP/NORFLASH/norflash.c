...
void norflash_init(void)
{
    ...
    if (g_norflash_type == W25Q256)         /* SPI FLASH为W25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)             /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            NORFLASH_CS(0);
            spi5_read_write_byte(FLASH_Enable4ByteAddr);    /* 使能4字节地址指令 */
            NORFLASH_CS(1);
        }
    }
...
{
    NORFLASH_CS(0);
    spi5_read_write_byte(FLASH_WriteEnable);   /* 发送写使能 */
    NORFLASH_CS(1);
}
...
static void norflash_send_address(uint32_t address)
{
    ...
    {
        spi5_read_write_byte((uint8_t)((address)>>24)); /* 发送 bit31 ~ bit24 地址 */
    } 
    spi5_read_write_byte((uint8_t)((address)>>16));     /* 发送 bit23 ~ bit16 地址 */
    spi5_read_write_byte((uint8_t)((address)>>8));      /* 发送 bit15 ~ bit8  地址 */
    spi5_read_write_byte((uint8_t)address);             /* 发送 bit7  ~ bit0  地址 */
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    NORFLASH_CS(0);
    spi5_read_write_byte(command);      /* 发送读寄存器命令 */
    byte = spi5_read_write_byte(0Xff);  /* 读取一个字节 */
    NORFLASH_CS(1);
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    NORFLASH_CS(0);
    spi5_read_write_byte(command);  /* 发送读寄存器命令 */
    spi5_read_write_byte(sr);       /* 写入一个字节 */
    NORFLASH_CS(1);
}
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    spi5_read_write_byte(FLASH_ManufactDeviceID);   /* 发送读 ID 命令 */
    spi5_read_write_byte(0);                        /* 写入一个字节 */
    spi5_read_write_byte(0);
    spi5_read_write_byte(0);
    deviceid = spi5_read_write_byte(0xFF) << 8;     /* 读取高8位字节 */
    deviceid |= spi5_read_write_byte(0xFF);         /* 读取低8位字节 */
    NORFLASH_CS(1);
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    spi5_read_write_byte(FLASH_ChipErase);  /* 发送读寄存器命令 */ 
    NORFLASH_CS(1);
